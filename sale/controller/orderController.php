...
if(isset($_GET['new']) && isset($_POST['id_customer']) && isset($_GET['id_sale']) )
{
	...
	$isExists 		= $cart->isActiveCartExists($id_sale);
	$content 			= $_GET['content'];
	if($isExists)
	{
		$cart->setCustomer($cart->id_cart, $id_customer);
	}
	else
	{
		$arr = array(
...
if(isset($_GET['confirm_order']) && isset($_POST['id_cart']))
{
	$sc = TRUE;
	...
	$qs = dbQuery("SELECT * FROM tbl_cart_product WHERE id_cart = ".$id_cart." AND valid = 0");
	...
	if(dbNumRows($qs) > 0)
	{
		...
		$order = new order();
		$cart = new cart();
		$id_order = $order->add($arr);
		if($id_order === FALSE)
		{
			$sc = FALSE;
			$message = 'เพิ่มออเดอร์ไม่สำเร็จ กรุณาลองใหม่อีกครั้ง';
		}
		else
		{
			...
			{
				if($order->insertDetail($rs->id_product_attribute, $rs->qty) === FALSE)
				{
					$sc = FALSE;
					$message = 'เพิ่มรายการ '.$rs->id_product_attribute.' ไม่สำเร็จ กรุณาลองใหม่อีกครั้ง';
				}
				else
				{
					if($cart->valid_detail($rs->id_cart_product) === FALSE)
					{
						$sc = FALSE;
						$message = 'เปลียนสถานะ cart product ไม่สำเร็จ';
					}
				}
			}
			...
			if($sc === TRUE)
			{
				if($cart->valid_cart($id_cart) === FALSE)
				{
					$sc = FALSE;
					$message = 'เปลี่ยนสถานะ ตะกร้าสินค้าไม่สำเร็จ';
				}
			}
		}
...
// 			$qty = $rs['qty'];
// 			$stock_qty = $product->available_order_qty($id_product_attribute) + $qty;
// 			if($qty > $stock_qty){
...
// 		}else{
// 			$reference = get_max_role_reference("PREFIX_ORDER",1);
// 			$qr = dbQuery("INSERT INTO tbl_order (reference, id_customer, id_employee, id_cart, current_state, payment, comment, valid, role, date_add, order_status)
//			VALUES ('".$reference."', ".$id_customer.", ".$id_employee.", ".$id_cart.", 1, 'เครดิต', '".$comment."', 0, 1, NOW(), 1)");
// 			$id_order = dbInsertId();
// 			$order = new order($id_order);
// 			foreach($data as $rs)
// 			{
...
if(isset($_GET['clearFilter']) && isset($_GET['tracking']))
{
	deleteCookie('sCode');
	deleteCookie('sName');
