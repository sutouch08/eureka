...
class cart{
		...
		public function valid_detail($id_cart_product)
		{
			return dbQuery("UPDATE tbl_cart_product SET valid = 1 WHERE id_cart_product = ".$id_cart_product);
		}


		public function valid_cart($id_cart)
		{
			return dbQuery("UPDATE tbl_cart SET valid = 1 WHERE id_cart = ".$id_cart);
		}

		public function isActiveCartExists($id_sale)
		{
			$qs = dbQuery("SELECT id_cart FROM tbl_cart WHERE id_sale = ".$id_sale." AND valid = 0");
			if(dbNumRows($qs) > 0)
			{
				$rs = dbFetchObject($qs);
		...
		public function add(array $ds = array())
		{
			if(!empty($ds))
			{
				...
				$qr = "INSERT INTO tbl_cart (".$fields.") VALUES (".$values.")";
		...
		{
			$qs = dbQuery("SELECT * FROM tbl_cart WHERE id_cart = ".$id);
			if(dbNumRows($qs) == 1)
			{
		...
		public function setCustomer($id_cart, $id_customer)
		{
			return dbQuery("UPDATE tbl_cart SET id_customer = ".$id_customer." WHERE id_cart = ".$id_cart);
		}



		public function getCart($id_sale)
		{
		...
		public function getCartDetails($id_cart)
		{
			$qr  = "SELECT cd.id_cart_product, cd.id_cart, ";
			$qr .= "pa.id_product, pa.id_product_attribute, cd.qty ";
			$qr .= "FROM tbl_cart_product AS cd ";
			$qr .= "LEFT JOIN tbl_product_attribute AS pa ON cd.id_product_attribute = pa.id_product_attribute ";
			$qr .= "LEFT JOIN tbl_size AS si ON pa.id_size = si.id_size ";
			$qr .= "WHERE cd.id_cart = ".$id_cart." ";
			$qr .= "ORDER BY pa.id_product ASC, ";
			$qr .= "pa.id_color ASC, ";
			$qr .= "si.position ASC";
		...
		public function cart_detail($id_cart)
		{
			if($id_cart != "" )
			{
				$sql  = "SELECT cp.id_cart_product, cp.id_cart, cp.id_product_attribute, cp.qty ";
				$sql .= "FROM tbl_cart_product AS cp ";
				$sql .= "JOIN tbl_product_attribute AS pa ON cp.id_product_attribute = pa.id_product_attribute ";
				$sql .= "LEFT JOIN tbl_size AS si ON pa.id_size = si.id_size ";
				$sql .= "WHERE id_cart = ".$id_cart." ";
				$sql .= "ORDER BY pa.id_product ASC, pa.id_color ASC, si.position ASC";
