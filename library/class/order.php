...
class order{
	...
	public function getTotalAmount($id_order)
	{
		$qr = "SELECT SUM(total_amount) AS amount FROM tbl_order_detail WHERE id_order = ".$id_order;
		$qs = dbQuery($qr);
		if(dbNumRows($qs) == 1)
		{
			$rs = dbFetchObject($qs);
...
public function state_change($id_order, $id_order_state, $id_employee){
	...
	public function orderProductTable($can_edit = "",$can_delete="")
	{
		//แสดงรายการสินค้า
	...
	public function qc_amount($id_order=""){
		$amount = 0;
		$id_order = $id_order == '' ? $this->id_order : $id_order;
	...
	public function prepared_amount($id_order)
	{
		$amount = 0;
	...
	{
		$qr = "SELECT discount_amount FROM tbl_order_discount WHERE id_order = '".$id_order."'";
		$qs = dbQuery($qr);
		if(dbNumRows($qs) == 1)
		{
			$rs = dbFetchObject($qs);
