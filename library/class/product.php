...
public function order_qty($id_pa = '')
{
	$id_pa = $id_pa == '' ? $this->id_product_attribute : $id_pa;
	$qr  = "SELECT SUM(product_qty) AS qty FROM tbl_order_detail AS od ";
	$qr .= "LEFT JOIN tbl_order AS o ON od.id_order = o.id_order ";
	$qr .= "WHERE id_product_attribute = ".$id_pa." ";
	$qr .= "AND valid_detail = 0 ";
	$qr .= "AND current_state NOT IN(6,7,8,9)";
	...
	public function cart_qty($id_pa = '')
	{
		$id_pa = $id_pa == '' ? $this->id_product_attribute : $id_pa;

		$qr  = "SELECT SUM(qty) AS qty FROM tbl_cart_product ";
		$qr .= "WHERE id_product_attribute = ".$id_pa." ";
		$qr .= "AND valid = 0 ";

		$qs = dbQuery($qr);

		if($rs = dbNumRows($qs) > 0)
		{
			$rs = dbFetchObject($qs);
			return is_null($rs->qty) ? 0 : $rs->qty;
		}
...
public function orderProductQty($id_product)
{
	...
	$qr .= "WHERE od.id_product = ".$id_product." ";
	$qr .= "AND valid_detail = 0 ";
	$qr .= "AND o.current_state NOT IN(6,7,8,9)";
...
public function available_order_qty($id_product_attribute, $id_order='')
{
	...
	$cart_qty = $this->cart_qty($id_product_attribute);
...
private function orderAttributeGridOneAttribute($id_pd, $attr, $isVisual, $id_order = '')
{
	...
	while($rs = dbFetchArray($data))
	{
		...
		$sc 	.= '<input type="number" class="form-control" title="'.$id_pa.'" name="qty[0]['.$id_pa.']" id="qty_'.$id_pa.'" onkeyup="valid_qty($(this), '.($qty === FALSE ? 1000000 : $qty).')" '.$disabled.' />';
		$sc 	.= '</td>';
		...
		while($rs = dbFetchArray($qs))
		{
			...
			while($rd = dbFetchArray($qr))
			{
				...
				if( $id_pa !== FALSE)
				{
					...
					//$orderQty 	= $isVisual === FALSE && $active == 1 ? $this->orderQty($id_pa) : 0; //--- ออเดอร์ค้างส่
					...
					//$sc 	.= $isVisual === FALSE ? '<center><span style="color:blue; font-size:10px;">('.$all_qty.')</span></center>' : '';
					$sc 	.= '<input type="number" class="form-control" title="'. $id_pa .'" name="qty['.$rd['id'].']['.$id_pa.']" id="qty_'.$id_pa.'" onkeyup="valid_qty($(this), '.($qty === FALSE ? 1000000 : $qty).')" '.$disabled.' />';
...
public function order_attribute_grid($id_product, $id_order = '')
{
	...
	else if($c_count == "111") /// กรณีมี 3 คุณลักษณะ
	{
		...
		$qr  = "SELECT pa.id_attribute AS id, ";
		$qr .= "at.attribute_name AS name ";
		$qr .= "FROM tbl_product_attribute AS pa ";
		$qr .= "JOIN tbl_attribute AS at ON pa.id_attribute = at.id_attribute ";
		$qr .= "WHERE pa.id_product = ".$id_product." ";
		$qr .= "AND pa..id_attribute != 0 ";
		$qr .= "GROUP BY pa.id_attribute ";
		$qr .= "ORDER BY at.position ASC";
