...
{
	$qr  = "SELECT id_customer FROM tbl_customer ";
...
function featureProduct($i, $id_cus=0)
{
	...
	$qr .= "pc.product_detail ";
	$qr .= "FROM tbl_product AS pd ";
	$qr .= "LEFT JOIN tbl_product_detail AS pc ON pd.id_product = pc.id_product ";
	$qr .= "LEFT JOIN tbl_category_product AS cp ON pd.id_product = cp.id_product ";
	$qr .= "WHERE pd.active = 1 ";
	$qr .= "AND cp.id_category = 0 ";
	$qr .= "ORDER BY pd.date_add DESC ";
	$qr .= "LIMIT ".$i;
	...
	if(dbNumRows($qs) > 0)
	{
		...
		{
			$product = new product();
			$discount = $rs->discount > 0 ? ($rs->discount_type == 'percentage' ? $rs->product_price * ($rs->discount * 0.01) : $rs->discount) : 0;
			$product_sell = $rs->product_price - $discount;

			$sc .= '<div class="item2 col-lg-3 col-md-3 col-sm-6 col-xs-12">';
			$sc .= 	'<div class="product">';
			$sc .= 		'<div class="image">';
			$sc .= 			'<a href="#" onclick="getData('.$rs->id_product.')">'.$product->getCoverImage($rs->id_product,3,'img-responsive').'</a>';
			$sc .= 		'</div>';
			$sc .= 		'<div class="description" style="min-height:60px;">';
			$sc .= 			'<a href="#" onclick="getData('.$rs->id_product.')">'.$rs->product_code.' <br/>'.$rs->product_name.'</a>';
			$sc .= 		'</div>';
			$sc .= 		'<div class="price">';
			$sc .= 			'<span class="old-price">'.($rs->discount > 0 ? number_format($rs->product_price, 2) : '&nbsp;').'</span>';
			$sc .= 		'</div>';
			$sc .= 		'<div class="action-control">';
			$sc .=     	'<a href="#" data-toggle="modal" data-target="#'.$rs->id_product.'">';
			$sc .= 		 		'<span class="btn btn-primary" style="width:80%;">'.number_format($product_sell,2).' ฿</span>';
			$sc .= 			'</a>';
			$sc .= 		'</div>';
			$sc .=	'</div>';
			$sc .= '</div>';
...
function order_state_change($id_order, $state, $id_emp)
{
	...
	if($state == 2)
	{
		if( $order->validOrder($id_order) === FALSE)
		{
			$sc = FALSE;
	...
	else if($state == 1 OR $state == 3 )
	{
		...
		{
			if(rollback_order($id_order) === FALSE)
			{
				$sc = FALSE;
				$message = 'Roll back order not success';
			}  ///-----  ย้อนกระบวนการ  ----///
		}
	...
	else if($state == 8 )
	{
		...
		else if($c_state == 11 || $c_state == 10 || $c_state == 5 || $c_state == 4 || $c_state == 3 || $c_state == 1)
		{
			...
			if( ! clear_buffer($id_order) )
			{
				$sc = FALSE;
				$message = 'Fail to Clear buffer';
			}
			...
			{
				$support = new support();
				$sp = $support->getOrderSupport($id_order);
				$support->update_budget($sp->id_budget, $amount);
				set_order_support_amount($id_order, 0.00 );
				update_order_support_status($id_order, 2);   /// 0 = notvalid   1 = valid  2 = cancle
			}
			else if($order->role == 4 )
			{
				$sponsor = new sponsor();
				$sp = $sponsor->getOrderSponsor($id_order);
				$sponsor->update_budget($sp->id_budget, $amount);
				set_order_sponsor_amount($id_order, 0.00 );
...
function rollback_order($id_order)
{
	...
	else
	{
		...
		$qs = dbQuery("SELECT * FROM tbl_order_detail_sold WHERE id_order = ".$id_order);
		//----- ถ้ามียอดขาย
		if( dbNumRows($qs) > 0 )
		{
			while( $rs = dbFetchObject($qs) )
			{
				$qm = $movement->getMovement($rs->reference, $rs->id_product_attribute);
				...
				if( dbNumRows($qm) > 0 )
				{
					...
					while( $rm = dbFetchObject($qm) )
					{
						...
						if( ! $buffer->updateBuffer($id_order, $rs->id_product, $rs->id_product_attribute, $rm->id_zone, $rm->id_warehouse, $rm->move_out))
						{
							$sc = FALSE;
							$message = 'Fail to update buffer';
						}
					}

					if( ! $movement->removeMovement($rs->reference, $rs->id_product_attribute))
					{
						$sc = FALSE;
						$message = 'Cannot remove movement';
					}

					if(! $order->unSoldProduct($id_order, $rs->id_product_attribute))
					{
						$sc = FALSE;
						$message = 'Cannot remove sold data';
					}
		...
		if($order->role == 7)
		{
			...
			if( $order_amount != $qc_amount )
			{
				...
				if( ! $support->update_budget($id_budget, $amount) )
				{
					$sc = FALSE;
					$message = 'ปรับปรุงยอดงบประมาณไม่สำเร็จ';
				}
			}
			...
			update_order_support_status($id_order, 0);
		...
		if($order->role == 4)
		{
			...
			$order_amount 	=  $order->getCurrentOrderAmount($id_order); 	//----- ตรวจสอบยอดเงินสั่งซื้อ
			$qc_amount 		= $sp->amount;						//----- ตรวจสอบยอดเงินที่ qc ได้

			//----- ถ้าไม่เท่ากันให้ทำการปรับปรุงยอดงบประมาณคงเหลือ
			if($order_amount != $qc_amount)
			{
				$id_budget 	= $sp->id_budget;

				//----- ยอดต่างระหว่างยอดเงินสั่งซื้อ กับ ยอดเงิน qc  แล้วทำให้ติดลบเพื่อทำการลดงบประมาณให้เป็นไปตามออเดอร์ เพราะเมื่อเปิดบิลอีกครั้งหากมียอดต่างจะบวกกลับให้
				$amount 		= ($order_amount - $qc_amount) * -1;

				//-----  ปรับปรุงยอดงบประมาณคงเหลือ
				if( ! $sponsor->update_budget($id_budget, $balance) )
				{
					$sc = FALSE;
					$message = 'ปรับยอดงบประมาณไม่สำเร็จ';
				}
			...
			update_order_sponsor_status($id_order, 0);
...
function cancle_order($id_order)
{
	...
	else
	{
		...
		if( dbNumRows($sql) > 0 )
		{
			...
			{
				$qr = $movement->getMovement($rs->reference, $rs->id_product_attribute);

				if( dbNumRows($qr) > 0 )
				{
					//----- ทำการเพิ่มรายการเข้า tbl_cancle
					while($rm = dbFetchObject($qr) )
					{
						$arr = array(
							'id_order' => $id_order,
						...
						{
							$sc = FALSE;
							$message = 'ย้ายสินค้าเข้า cancle zone ไม่สำเร็จ';
						}

					} //-- end while

					//----- ลบ stock_movement
					if(! $movement->removeMovement($rs->reference, $rs->id_product_attribute, $rs->id_zone))
					{
						$sc = FALSE;
			...
			if($order->role == 4 )
			{
				...
				//--- คืนยอดงบประมาณ
				$sponsor->update_budget($sp->id_budget, $sp->amount);

				//----- ปรับปรุงยอดออเดอร์ใน order_sponsor
				set_order_sponsor_amount($id_order, 0.00 );

				//----- 0 = notvalid   1 = valid  2 = cancle
				update_order_sponsor_status($id_order, 2);
...
function clear_buffer($id_order)
{
	...
	//---- ดึงยอดที่เหลือเก็บไว้ก่อน
	$qs = $buffer->getBuffer($id_order);

	//---- ลบ buffer ทั้งหมด
	if($buffer->clearBuffer($id_order))
	{
		if(dbNumRows($qs) > 0)
		{
			while($rs = dbFetchObject($qs))
...
function customer_name($id_customer){
	...
	$qs = dbQuery("SELECT first_name, last_name, company FROM tbl_customer WHERE id_customer = ".$id_customer);
	if(dbNumRows($qs) == 1)
	{
		$rs = dbFetchArray($qs);
		if($rs['company'] == "")
...
function return_budget($id_order)
{
	...
	if($order->current_state == 9 )
	{
		...
		$amount = $order->prepared_amount($id_order);
	}
	else
	{
		$amount = $order->getCurrentOrderAmount($id_order);
