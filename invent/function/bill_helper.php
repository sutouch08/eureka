...
function get_sold_qty($id_order, $id_pa)
{
	$rs = false;
	$role = order_role($id_order);
	if($role == 5 )
	{
		$qr  = "SELECT qty FROM tbl_order_consignment_detail WHERE id_order = ".$id_order." AND id_product_attribute = ".$id_pa;
	}
	else
	{
		$qr = "SELECT sold_qty AS qty FROM tbl_order_detail_sold WHERE id_order = ".$id_order." AND id_product_attribute = ".$id_pa;
	}
...
function get_sold_data($id_order, $id_product_attribute)
{
	...
	if($role == 5 )
	{
		$qr  = "SELECT cm.qty AS sold_qty, reduction_percent, reduction_amount ";
		$qr .= "FROM tbl_order_consignment_detail AS cm ";
		$qr .= "JOIN tbl_order_detail AS od ON cm.id_order = od.id_order ";
		$qr .= "AND cm.id_product_attribute = od.id_product_attribute ";
		$qr .= "WHERE cm.id_order = ".$id_order." ";
		$qr .= "AND cm.id_product_attribute = ".$id_product_attribute." ";
		$qs = dbQuery($qr);
	}
	else
	{
